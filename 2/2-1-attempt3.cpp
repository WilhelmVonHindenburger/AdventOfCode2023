//Advent of Code 2023, Problem 2, Part 1, Attempt 3
  ...
  */
  ...
  for(int i=0; i<allGames.size(); i++){
    ...
    for(int j=0; j<allGames[i].draws.size(); j++){
      ...
      if((allGames[i].draws[j].red > redBalls) || (allGames[i].draws[j].green > greenBalls) || (allGames[i].draws[j].blue > blueBalls)){
        cout << "Game " << allGames[i].id << " is impossible.\n";
        gamePossible=false;
      }
    }
    if(gamePossible){
      total += allGames[i].id;
      cout << "Game " << allGames[i].id << " is POSSIBLE.\n";
    }
  }
